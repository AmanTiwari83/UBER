        ...
        <div className='flex gap-2 justify-between  flex-col '>
          ...
          <div className='mt-6 p-0 '>
            <form action="" className="" onSubmit={(e) => submitHandler(e)}>
              <h3 className='text-xl text-gray-700 font-bold'>Enter the OTP</h3>
              <input type="text" placeholder='Enter OTP' onChange={(e) => setOtp(e.target.value)} value={otp} className='bg-[#eee] px-4 py-3 text-base rounded-lg w-full mt-5 ' />
              <Link to="/captain-riding" className='w-full flex justify-center mt-7 bg-green-600 text-white font-s p-2 rounded-lg '>Confirm Ride
              </Link>
              <button onClick={() => {
                setRidePopUpPanel(false)
                setConfirmRidePopUpPanel(false)
              }} className='w-full mt-3 text-white bg-red-500 font-semibold p-2 rounded-lg '>
                Cancel Ride
              </button>
            </form>
          </div>
